    ...
    def run_terminal(self, profiles: list[dict], auto: bool = False, max_concurrent_profiles: int = 4, headless: bool = False):
        ...
        Args:
            profiles (list[dict]): Danh sách các profile trình duyệt có thể khởi chạy.
                Mỗi profile là một dictionary chứa thông tin, trong đó key 'profile' là bắt buộc. 
                Ví dụ: {'profile': 'profile_name', ...}
            auto (bool, option): True, bỏ qua lựa chọn terminal và chạy trực tiếp chức năng auto. Mặc định False.
            max_concurrent_profiles (int, option): Số lượng tối đa các hồ sơ có thể chạy đồng thời. Mặc định là 4.
            headless (bool, option): True, sẽ ẩn duyệt trình khi chạy. Mặc định False.
